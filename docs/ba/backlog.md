...
| BL-5 | `MatchingAlgorithm` ABC, `ParamSpec`, `AlgorithmOutput`, `AlgorithmFamily` (`app/algorithms/base.py`) | M2 | Must | Planned |
| BL-6 | Algorithm registry (`app/algorithms/registry.py`) | M2 | Must | Planned |
| BL-7 | `FingerprintAlgorithm` (key-collision, no params) | M2 | Must | Planned |
| BL-8 | `NGramFingerprintAlgorithm` (key-collision, param `n`, default 2) | M2 | Must | Planned |
| BL-9 | `matching_service.run_matching` — key-collision orchestration path | M2 | Must | Planned |
| BL-10 | `GET/POST /algorithm` router + template | M2 | Must | Planned |
| BL-11 | `GET /results` router + template (read-only; accept/reject inert) | M2 | Must | Planned |
| BL-12 | Resolve mapping->algorithm redirect target (OQ-M2-1) | M2 | Should | Open question |
| BL-13 | `compute_blocks` (zip-prefix / city fallback / shared-bucket blocking) | M3 | Must | Planned |
| BL-14 | `normalized_compression_distance` (NCD, bz2-based, order-averaged) | M3 | Must | Planned |
| BL-15 | `LevenshteinNNAlgorithm` (rapidfuzz, param `threshold`, default 3) | M3 | Must | Planned |
| BL-16 | `NCDAlgorithm` (PPM-approx, UI threshold default 3, internal scaling /10) | M3 | Must | Planned |
| BL-17 | Extend `matching_service` with nearest-neighbor path + union-find clustering | M3 | Must | Planned |
